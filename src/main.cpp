...
static const int64_t nTargetTimespan = 16 * 60; // 16m diff retarget
unsigned int nTargetSpacing = 2 * 60; // 2 minutes block time
...
const string strMessageMagic = "celerocoin Signed Message:\n";
...
const int QUARTER_BLOCKCOUNT = 65700;
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    int64_t nRewardCoinYear = COIN_YEAR_REWARD;
	
	if(nBestHeight < QUARTER_BLOCKCOUNT)
        nRewardCoinYear = 3.9 * COIN_YEAR_REWARD;
    else if(nBestHeight < (2 * QUARTER_BLOCKCOUNT))
        nRewardCoinYear = 3.6 * COIN_YEAR_REWARD; 
    else if(nBestHeight < (3 * QUARTER_BLOCKCOUNT))
        nRewardCoinYear = 3.3 * COIN_YEAR_REWARD;
    else if(nBestHeight < (4 * QUARTER_BLOCKCOUNT))
        nRewardCoinYear = 3 * COIN_YEAR_REWARD;
    else if(nBestHeight < (5 * QUARTER_BLOCKCOUNT))
        nRewardCoinYear = 2.7 * COIN_YEAR_REWARD; 
    else if(nBestHeight < (6 * QUARTER_BLOCKCOUNT))
        nRewardCoinYear = 2.4 * COIN_YEAR_REWARD; 
    else if(nBestHeight < (7 * QUARTER_BLOCKCOUNT))
         nRewardCoinYear = 2.1 * COIN_YEAR_REWARD; 
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // celerocoin: check proof-of-stake block signature
...
// celerocoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "celerocoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "02nov2017";
        CTransaction txNew;
        txNew.nTime = 1509580801;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1509580801;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 60537 : 60537;
...
unsigned char pchMessageStart[4] = { 0x83, 0x5f, 0x8f, 0x3c };
